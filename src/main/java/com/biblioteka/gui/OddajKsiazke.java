    ...
    Oddaj.addActionListener(e -> {
      ...
      try {
        ...
        if (wypozyczoneKsiazki.containsKey(id)) {
          ...
          JOptionPane.showMessageDialog(null, "Nie masz wypożyczonej tej książki!", "Błąd oddania",
              JOptionPane.ERROR_MESSAGE);
        }

      } catch (NumberFormatException exep) {
        JOptionPane.showMessageDialog(null, "Niepoprawny napis!", "Błąd oddania",
            JOptionPane.ERROR_MESSAGE);
      }
    });
    add(Oddaj);
  ...
  public void update() {
    ...
    if (zalogowanyUżytkownik != null) {
      ...
      for (int i = 0; i < wypozyczoneKsiazki.size(); i++) {
        ...
        tableModel.addRow(
            new Object[] {index[i], b[i].getTytuł(), b[i].getAutor(), b[i].getRokWydania()});
      }
